...
class OnlinestoreClient(object):
    '''Client Onlinestore class.

    This supports buy, sell, transport, and show functions.
    '''
    ...
    def __init__(self, baseUrl, keyFile=None):
        ...
        '''Address is 6-char TF prefix + hash of "onlinestore"'s public key'''
    ...
    # 2. Send to rest-api
    def buy(self, value):
        return self._wrap_and_send("buy",value)

    def sell(self, value):
        try:
            retValue = self._wrap_and_send("sell", value)
        except Exception:
            raise Exception('Encountered an error during withdraw')
        return retValue
    ...
    def transfer(self, value, clientToKey):
        try:
            ...
            retValue = self._wrap_and_send("transport", value, publicKeyStr)
        except OSError as err:
            raise Exception('Failed to read public key {}: {}'.format(
                clientToKey, str(err)))
        except Exception as err:
            raise Exception('Encountered an error during transport', err)
        return retValue
    ...
    def clear(self):
        ...
        try:
            ret_amount = self._wrap_and_send("empty", 0)
        except Exception:
